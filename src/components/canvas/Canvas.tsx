...
// Week 1：freedraw 加入可绘制工具
const DRAWABLE_TOOLS: readonly DrawableTool[] = [
    'rectangle',
    'ellipse',
    'line',
    'arrow',
...
export function Canvas({
    ...
    const onPointerMove = (e: React.PointerEvent) => {
        ...
        if (interaction.type === 'draft') {
            if (interaction.element.type === 'freedraw') {
                const pressure = e.pressure > 0 ? e.pressure : 0.5;
                interactionRef.current = {
                    ...interaction,
                    element: pushFreedrawPoint(interaction.element, p.x, p.y, pressure),
                };
            } else {
                interactionRef.current = {
                    ...interaction,
        ...
        if (interaction.type === 'resize') {
            const local = interaction.startAngle
                ? rotatePoint(
                      p.x,
                      p.y,
                      interaction.startCenter.cx,
                      interaction.startCenter.cy,
    ...
    const onPointerUp = (e: React.PointerEvent) => {
        ...
        if (interaction.type === 'draft') {
            ...
            // Week 1：freedraw 用最小点数判定，其它元素走面积判定
            if (draft.type === 'freedraw') {
                if (draft.points.length < 2) {
                    invalidate();
                    return;
                }
            } else if (Math.abs(draft.width) < 2 && Math.abs(draft.height) < 2) {
                invalidate();
                return;
            }
